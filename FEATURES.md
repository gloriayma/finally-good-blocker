...
and `extra` labels. The `Sites` heading stays centered, while saved hostnames are
left-justified with an inset from the table edge. Remove actions remain centered
